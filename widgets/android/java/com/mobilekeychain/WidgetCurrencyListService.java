...
public class WidgetCurrencyListService extends RemoteViewsService {
    ...
    class AppWidgetItemFactory implements RemoteViewsFactory {
        ...
        public void onDataSetChanged() {
            ...
            JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, KEYCHAIN_PRICE_API_URL,
            ...
            try {
                ...
                JsonObjectRequest requestHistoryPrices = new JsonObjectRequest(Request.Method.GET,
                        KEYCHAIN_PRICE_HISTORY_API_URL, new JSONObject(), futureHistoryPrices, futureHistoryPrices);
                ...
                if (e.getLocalizedMessage().contains("Unable to resolve host")) {
                    ...
                    views.setTextViewText(R.id.widget_currency_list_stack_empty_view,
                            "Please enable internet & retry refresh");
                    views.setTextViewTextSize(R.id.widget_currency_list_stack_empty_view, 1, 12);
                    AppWidgetManager widgetManager = AppWidgetManager.getInstance(context);
                    ComponentName widgetComponentCurrency = new ComponentName(context,
                            WidgetCurrencyListProvider.class);
        ...
        public RemoteViews getViewAt(int position) {
            ...
            try {
                ...
                String currency_usd_24h_change_value = String.format("%.2f",
                        valuesJsonObject.getDouble("usd_24h_change")) + "%";
                // validate & remove the -
                String currency_change_no_minus = currency_usd_24h_change_value.contains("-")
                        ? currency_usd_24h_change_value.split("\\-")[1]
                        : currency_usd_24h_change_value;
                // set values for currency item
                views.setTextViewText(R.id.widget_currency_list_item_currency_name,
                        currency_name.contains("hive dollar") ? "HBD" : currency_name.toUpperCase());
                views.setTextViewText(R.id.widget_currency_list_item_currency_value_usd, currency_value_usd);
                views.setTextViewText(R.id.widget_currency_list_item_currency_usd_24h_change_value,
                        currency_change_no_minus);
                // set icon dynamically
                views.setImageViewResource(R.id.widget_currency_list_currency_icon,
                        !currency_name.contains("hive dollar") ? R.drawable.hive_icon : R.drawable.hbd_icon);
                // New Chart
                String currency_tendency = currency_usd_24h_change_value.contains("-") ? "down" : "up";
                Bitmap currency_chart = drawChart(280.0f, 100.0f, currency_name, false, false, true, currency_tendency);
                // set remote view
                ...
                if (currency_usd_24h_change_value.contains("-")) {
                    views.setTextColor(R.id.widget_currency_list_item_currency_usd_24h_change_value,
        ...
        public Bitmap drawChart(float bitmap_width, float bitmap_height, String currency_name, boolean drawPoints,
                boolean draw_as_bars, boolean drawExtraLines, String tendency) throws JSONException {
            // Data fetched array + calculations
            if (currency_name.contains("hive dollar"))
                currency_name = "hbd";
            ...
            Bitmap bitmap = Bitmap.createBitmap(Math.round(bitmap_width), Math.round(bitmap_height),
            ...
            Integer graph_color = Color.parseColor("#00b200");
            ...
            if (drawExtraLines)
                paint_chart.setColor(graph_line_darker_color);
            while (li.hasNext()) {
                if (li.hasNext()) {
                    FloatPoint secondPoint = li.next();
                    // bellow using bezier
                    FloatPoint connection_point_1 = new FloatPoint((secondPoint.getX() + firstPoint.getX()) / 2f,
                            firstPoint.getY());
                    FloatPoint connection_point_2 = new FloatPoint((secondPoint.getX() + firstPoint.getX()) / 2f,
                            secondPoint.getY());
                    if (drawExtraLines)
                        canvas_chart.drawLine(firstPoint.getX(), firstPoint.getY(), secondPoint.getX(),
                                secondPoint.getY(), paint_chart);
                    if (init_path) {
                        ...
                        // bellow using bezier
                        graph_path.cubicTo(
                                connection_point_1.getX(), connection_point_1.getY(), connection_point_2.getX(),
                                connection_point_2.getY(),
            ...
            // gradient
            paint_chart.setShader(new LinearGradient(0f, 0f, 0f, bitmap_height / 2, Color.TRANSPARENT, graph_color,
        ...
        public Float getScaledValue(float value, float sourceRangeMin, float sourceRangeMax, float targetRangeMin,
