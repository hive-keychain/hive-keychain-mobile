...
describe('accounts actions', () => {
  ...
  describe('addKey', () => {
    it('should show error modal when keys are invalid', async () => {
      // Mock getAccounts to return account, but use invalid key
      (getClient as jest.Mock).mockReturnValue({
        database: {
          getAccounts: jest
            .fn()
            .mockResolvedValue([createMockAccount('user1')]),
        },
      });
      const thunk = addKey(
        'user1',
      ...
      expect(showModal).toHaveBeenCalledWith(
        'toast.keys.not_a_key',
        expect.any(String),
      );
    });

    it('should show error modal when key type does not match', async () => {
      // Use posting key but request active key
      (getClient as jest.Mock).mockReturnValue({
        database: {
          getAccounts: jest
            .fn()
      ...
      const thunk = addKey('user1', KeyTypes.active, TEST_POSTING);
      const getStateWithAccount = jest.fn(() => ({
        auth: {mk: 'masterkey'},
        accounts: [{name: 'user1', keys: {}}],
      }));
    ...
    it('should add key successfully with active key', async () => {
      (getClient as jest.Mock).mockReturnValue({
        database: {
          getAccounts: jest
            .fn()
      ...
      const thunk = addKey('user1', KeyTypes.active, TEST_ACTIVE);
      const getStateWithAccount = jest.fn(() => ({
        auth: {mk: 'masterkey'},
        accounts: [{name: 'user1', keys: {}}],
      }));
    ...
    it('should add key successfully with posting key', async () => {
      (getClient as jest.Mock).mockReturnValue({
        database: {
          getAccounts: jest
            .fn()
      ...
      const thunk = addKey('user1', KeyTypes.posting, TEST_POSTING);
      const getStateWithAccount = jest.fn(() => ({
        auth: {mk: 'masterkey'},
        accounts: [{name: 'user1', keys: {}}],
      }));
    ...
    it('should update only the target account when multiple accounts exist', async () => {
      (getClient as jest.Mock).mockReturnValue({
        database: {
          getAccounts: jest
            .fn()
      ...
      await thunk(mockDispatch, getStateWithMultipleAccounts, undefined);
      expect(updateAccountsCall).toBeDefined();
      // Verify updateAccounts was called and executed
      expect(EncryptedStorageUtils.saveOnEncryptedStorage).toHaveBeenCalled();
    });
  });
